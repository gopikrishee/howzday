...
export const firestoreService = {
  ...
  async fetchUserEntries(userId: string): Promise<MoodEntry[]> {
    ...
    try {
      const q = query(collection(db, path), orderBy('date', 'desc'), limit(60));
      ...
      if (isQuotaExceededError(error)) {
        notifyQuotaExceeded();
        return storageService.getEntries(userId);
      }
  ...
  subscribeTargetReactions(
    ...
    return onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => {
          const data = d.data();
          // Filter out and purge legacy "You Got This" / "💪" from database
          if (
            data.label?.toLowerCase() === 'you got this' ||
            data.emoji === '💪'
          ) {
  ...
  async purgeDeprecatedReactions(userId: string): Promise<void> {
    const path = 'status_reactions';
    try {
      // 1. Where current user is the target of the reaction
      const q1 = query(collection(db, path), where('targetUserId', '==', userId));
      const snap1 = await getDocs(q1);
      for (const d of snap1.docs) {
        const data = d.data();
        if (data.label?.toLowerCase() === 'you got this' || data.emoji === '💪') {
          await deleteDoc(d.ref).catch(() => {});
        }
      }

      // 2. Where current user was the sender of the reaction
      const q2 = query(collection(db, path), where('senderId', '==', userId));
      ...
      console.warn('Error purging deprecated reactions:', e);
    }
  },

  /**
   * Sends mobile notification to all accepted crood members whenever a user changes their mood.
   * Message: "Your crood's mood changed today"
   */
  async sendCroodMoodChangeNotifications(
    currentUser: User,
    croods: UserProfile[],
    dateStr: string
  ): Promise<void> {
    if (!currentUser || !croods || croods.length === 0) return;
    const senderName = currentUser.displayName || currentUser.email?.split('@')[0] || 'Your crood';
    const senderPhoto = currentUser.photoURL || undefined;
    const timestamp = Date.now();

    const promises = croods.map(async (crood) => {
      if (!crood.userId || crood.userId === currentUser.uid) return;
      const notifId = `moodchange_${dateStr}_${currentUser.uid}_${crood.userId}_${timestamp}`;
      const docRef = doc(db, 'crood_checkin_notifications', notifId);
      try {
        const notif: CroodCheckInNotification = {
          id: notifId,
          senderId: currentUser.uid,
          senderName,
          senderPhoto,
          receiverId: crood.userId,
          date: dateStr,
          message: "Your crood's mood changed today",
          read: false,
        ...
        console.warn(`Failed to send mood change notification to crood ${crood.userId}:`, err);
      }
    });
   ...
   */
  async sendCroodCheckInNotifications(
    currentUser: User,
    croods: UserProfile[],
    dateStr: string
  ): Promise<void> {
    return this.sendCroodMoodChangeNotifications(currentUser, croods, dateStr);
  },

  /**
   * Subscribes to check-in notifications targeted to this user for the specified date.
   */
  subscribeIncomingCheckInNotifications(
    userId: string,
    dateStr: string,
    onNext: (notifications: CroodCheckInNotification[]) => void,
    onError?: (err: unknown) => void
  ) {
    const q = query(
      collection(db, 'crood_checkin_notifications'),
    ...
    return onSnapshot(
      q,
      (snapshot) => {
        const list: CroodCheckInNotification[] = [];
        snapshot.forEach((d) => {
          const data = d.data();
          list.push({
            id: data.id || d.id,
            ...
            receiverId: data.receiverId,
            date: data.date,
            message: data.message || 'Your crood checked-in today',
            read: Boolean(data.read),
            createdAt: data.createdAt,
            updatedAt: data.updatedAt,
          });
      ...
      (error) => {
        console.warn('Error in subscribeIncomingCheckInNotifications:', error);
        if (onError) onError(error);
      }
    );
  },
  ...
  /**
   * Mark a check-in notification as read
   */
  async markCheckInNotificationAsRead(notifId: string): Promise<void> {
    try {
      const docRef = doc(db, 'crood_checkin_notifications', notifId);
