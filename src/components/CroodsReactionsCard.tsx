...
export const CroodsReactionsCard: React.FC<CroodsReactionsCardProps> = ({
  ...
  // Acknowledge unread reactions when viewed in the Moods tab
  useEffect(() => {
  ...
  return (
    <section
      ...
      className="mb-5 overflow-hidden rounded-3xl bg-white/85 backdrop-blur-md border border-indigo-100/90 shadow-sm p-4 transition-all duration-300"
    >
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500 font-medium">
              Friends reacted to your status today
            </p>
          </div>
        </div>
        {reactions.length > 0 && (
      ...
      {reactions.length === 0 ? (
        <div className="py-4 px-3 rounded-2xl bg-slate-50/70 border border-slate-200/60 text-center">
          <p className="text-xs font-semibold text-slate-600">
            Waiting for your Croods to react to your status in Feeds
          </p>
          <p className="text-[10px] text-slate-500 mt-0.5">
            When friends react, their names and emojis will appear here!
          </p>
        </div>
      ) : (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    {/* Friend Name & Reaction Description */}
                    <div className="min-w-0">
                      ...
                      <span className="text-[11px] font-semibold text-indigo-700 block truncate">
                        reacted with {reaction.label} {reaction.emoji}
                      </span>
                    </div>
                  </div>
