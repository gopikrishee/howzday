...
export const FeedsView: React.FC<FeedsViewProps> = ({
  ...
  // Calculate Crood Climate / Pulse (only friends who checked in TODAY)
  const loggedFriendsToday = friends.filter(
    (f) => f.latestMood && f.latestMoodDate === todayStr
  );
  const moodCounts = (
    [
      'happy',
    ...
    .map((level) => ({
      ...
      count: loggedFriendsToday.filter((f) => f.latestMood === level).length,
    }))
  ...
  return (
    <div className="flex-1 flex flex-col pb-6 space-y-4">
      ...
      <div className="p-4 sm:p-5 rounded-3xl bg-gradient-to-br from-indigo-50/95 via-white to-amber-50/90 border border-indigo-200 shadow-sm">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[11px] font-bold text-slate-500">
            {loggedFriendsToday.length} of {friends.length} checked in today
          </span>
        </div>
      ...
      {/* Mobile Push Notification Alert Banner */}
      {notificationService.isSupported() && pushPermission !== 'granted' && (
        <div className="p-3.5 sm:p-4 rounded-2xl bg-amber-50/90 border border-amber-300 shadow-2xs flex items-center justify-between gap-3">
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[11px] text-slate-600 truncate font-medium">
                Get notified when your croods change mood today
              </p>
            </div>
          </div>
          <button
            type="button"
            id="enable-mobile-push-btn"
            onClick={handleEnablePush}
            className="px-3.5 py-2 min-h-[38px] rounded-xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold shrink-0 shadow-2xs active:scale-95 transition-all cursor-pointer flex items-center gap-1.5"
          >
      ...
      {/* Push Notifications Enabled Status Pill */}
      {pushPermission === 'granted' && (
        <div className="px-3.5 py-2.5 rounded-xl bg-emerald-50/80 border border-emerald-200 shadow-2xs flex items-center gap-1.5 text-xs text-emerald-800 font-bold">
          <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
          <span>Mobile Mood Alerts: Active</span>
        </div>
      )}
        ...
        {friends.map((friend) => {
          ...
          return (
            <motion.div
              ...
              className="p-3.5 sm:p-4.5 rounded-2xl sm:rounded-3xl bg-white border border-slate-200 shadow-xs flex flex-col space-y-3 relative overflow-hidden"
            >
              ...
              <div className="flex items-center justify-between">
                ...
                {moodConfig && isToday ? (
                  ...
                  <span className="text-xs font-semibold text-slate-600 bg-amber-50 px-2.5 py-1 rounded-full border border-amber-200/80 flex items-center gap-1 shadow-2xs">
              ...
              <div
                ...
                className="p-3 rounded-2xl bg-slate-50/90 border border-slate-200/90 text-left flex flex-col gap-1.5 transition-all shadow-2xs"
              >
                <div className="flex items-center gap-2">
              ...
              <div className="relative flex flex-col gap-2.5 pt-2 border-t border-slate-100 text-xs">
                      ...
                      {CHEER_OPTIONS.map((opt) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex flex-col items-center justify-center py-1.5 px-2 rounded-xl font-bold transition-all cursor-pointer border active:scale-95 shadow-2xs min-w-[42px] sm:min-w-[46px] min-h-[44px] ${
                              isSelected
                            ...
                            title={opt.label}
                          >
                            <span className="text-base sm:text-lg leading-none select-none">{opt.emoji}</span>
                            <span className="text-[9px] sm:text-[10px] font-bold leading-tight mt-0.5 text-slate-600 whitespace-nowrap select-none">
                              {opt.label}
                            </span>
                          </button>
                        );
                  ...
                  {!isToday && (
                    <div className="ml-auto">
                      <button
                        ...
                        disabled={isNudged}
                        className={`flex items-center gap-1.5 px-3.5 py-2 min-h-[40px] rounded-xl text-xs font-bold transition-all cursor-pointer shadow-2xs ${
                          isNudged
